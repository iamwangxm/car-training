...
@Component
public class CoursesManagerImpl extends BaseManagerImpl<Courses> implements CoursesManager {
	...
	@Override
	@Transactional(readOnly=true)
	public ResultPage<Courses> findPageByCourses(Courses autobotsComment, Integer pageSize, Integer pageNo) {
		ResultPage<Courses> resultPage = new ResultPage<Courses>();
	...
	@Override
	@Transactional(readOnly=true)
	public List<Courses> findListByCourses(Courses autobotsComment) {
		DetachedCriteria dc = detachedCriteria();
		dc.add(Restrictions.eq("enabled", true));
		dc.addOrder(Order.desc("createDate"));
